...
import {degToRad} from './utils/transformation';
{
  ...
  const arrays = createQuadAttributesArrays();
  const bufferInfo = twgl.createBufferInfoFromArrays(gl, arrays);

  // const texture = createCubemapTexture(gl);
  const textureSky = loadCubemapSkybox(gl);

  let u_matrix = mat4.create();
  let viewMatrix = mat4.create();
  let projectionMatrix = mat4.create();
  
  let u_viewDirectionProjectionInverse = mat4.create();
  ...
  function render(time: number) {
    ...
    mat4.invert(u_viewDirectionProjectionInverse, u_matrix);

    uniforms.u_matrix = u_matrix;
    uniforms.u_viewDirectionProjectionInverse = u_viewDirectionProjectionInverse;
...
function createQuadAttributesArrays() {
  return {
    position: {
      numComponents: 2,
      data: [
...
function loadCubemapSkybox(gl: WebGL2RenderingContext): WebGLTexture {
...
function createCubemapTexture(gl: WebGL2RenderingContext, tex?: WebGLTexture): WebGLTexture {
    ...
    // Texture init
    const cubeTexture = tex || gl.createTexture();
